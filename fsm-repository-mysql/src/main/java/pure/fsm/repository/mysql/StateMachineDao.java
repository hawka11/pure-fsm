...
public abstract class StateMachineDao implements GetHandle {
    ...
    private static final String PURE_FSM_LOCK_PREFIX = "pure.fsm.lock";
    ...
    public Set<String> getAllIds() {
        return getHandle().inTransaction((conn, status) -> {
            final Query<Map<String, Object>> query = conn.createQuery("SELECT id FROM purefsm_statemachine");
    ...
    public String getNextId() {
        return getHandle().inTransaction((conn, status) -> {
            final Query<Map<String, Object>> query = conn.createQuery("SELECT id FROM purefsm_statemachine_id FOR UPDATE");
            final Map<String, Object> first = query.first();
            final String id = first.get("id").toString();
            conn.update("UPDATE purefsm_statemachine_id SET id = id + 1");
            return id;
        });
    }
    ...
    public void insertStateMachineData(String smId, Transition transition) {
        getHandle().inTransaction((conn, status) -> {
            final Update statement = conn.createStatement("INSERT INTO purefsm_statemachine VALUES (:id, :transition)");
    ...
    public void updateStateMachineData(String smId, Transition transition) {
        getHandle().inTransaction((conn, status) -> {
            final Update statement = conn.createStatement("UPDATE purefsm_statemachine SET data = :transition WHERE id = :id");
            statement.bind("id", smId);
            statement.bind("transition", serializeTransition(transition));
            return statement.execute();
        });
    ...
    public boolean removeStateMachineData(String smId) {
        return getHandle().inTransaction((conn, status) -> {
            final Update statement = conn.createStatement("DELETE FROM purefsm_statemachine WHERE id = :id");
            statement.bind("id", smId);
    ...
    public Transition getStateMachineData(String smId) {
        return getHandle().inTransaction((conn, status) -> {
            final Query<Map<String, Object>> query = conn.createQuery("SELECT data from purefsm_statemachine WHERE id = :id");
            query.bind("id", smId);
